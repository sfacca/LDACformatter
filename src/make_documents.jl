...
function make_documents(bags::SparseMatrixCSC)
    ...
    # there are bags.n columns=documents
    for i in 1:bags.n
...
function write_documents(bags::Array{T,2}, name="bags.documents") where {T >: Number}
    write_documents(sparse(bags), name)
end
...
function _make_doc_line(vals, ids)
    ...
    elseif length(vals) == 0 #empty lines break readDocs
        ...
        str = str[1:end-1]#trailing whitespaces break readDocs
        str*="\n"
    end
    str
end
